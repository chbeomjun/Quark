...
impl Matrix4x4 {
    ...
    pub fn from_rotation_x(theta: f32) -> Self {
        let mut matrix = Self::identity();
        matrix.0[5] = theta.cos();
        matrix.0[6] = theta.sin();
        matrix.0[9] = -theta.sin();
        matrix.0[10] = theta.cos();
        matrix
    }

    pub fn from_rotation_y(theta: f32) -> Self {
        let mut matrix = Self::identity();
        matrix.0[0] = theta.cos();
        matrix.0[2] = theta.sin();
        matrix.0[8] = -theta.sin();
        matrix.0[10] = theta.cos();
        matrix
    }
    ...
    pub fn from_rotation_z(theta: f32) -> Self {
        let mut matrix = Self::identity();
        matrix.0[0] = theta.cos();
        matrix.0[1] = -theta.sin();
        matrix.0[4] = theta.sin();
        matrix.0[5] = theta.cos();
        matrix
    }
